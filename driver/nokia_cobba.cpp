...
void nokia_cobba_device::device_start()
{
	...
	m_microphone_gain = std::pow(10.0F, m_microphone_gain_db / 20.0F);
	m_earpiece_gain = std::pow(10.0F, m_earpiece_gain_db / 20.0F);
	...
	save_item(NAME(m_selected_microphone));
	save_item(NAME(m_selected_output));
...
void nokia_cobba_device::sound_stream_update(sound_stream &stream)
{
	...
	for (int index = 0; index < stream.samples(); ++index)
	{
		...
		const float analogue_input =
				m_selected_microphone < microphone_inputs
						? float(stream.get(m_selected_microphone, index))
						: 0.0F;
		const float microphone = std::clamp(
				analogue_input * m_microphone_gain,
		...
		if (m_selected_output < audio_outputs)
			stream.put(m_selected_output, index,
					float(sample) / converter_scale * m_earpiece_gain);
	}
}
