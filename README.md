...
| Audio, RF, network | Partial HLE | The laboratory network supports organic registration, T3212-driven periodic Location Updating, paging, MT call/SMS control, bidirectional GSM-FR TCH/F media, FACCH/SACCH coexistence, degraded frames and teardown. Idle-mode same-/different-LAC cell reselection, total-coverage loss/recovery and replacement-cell paging are validated; physical RF propagation, dedicated-mode handover, frequency hopping, wider services and DSP-local COBBA mux/control decoding remain open. |
