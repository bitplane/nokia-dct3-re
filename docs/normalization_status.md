...
| DSP/network/external-service | DSPIF, DSP HLE, Nokia L1 correlation, LAPDm, GSM session/network data and external service are separate owners. The generic network owns typed multi-cell topology, standards-shaped SI/control scheduling and RF conditions; the radio peer owns saved per-cell measurement/acquisition and dedicated-mode retune/access state while handset firmware owns selection, MM and handover completion. Same-/different-LAC reselection, replacement-cell paging, loss/recovery, preserved location and save-state gates pass across NSE-8/NHM-5/NHM-6/NHM-2. NSE-8 and NHM-5 independently prove successful handover, timed rollback and mid-handover save/load with product-specific confirmation values. Generic A5/0/A5/1 operates only on the 114-bit post-interleave burst seam; product DSP-control, Nokia neighbour grammar and PCM evidence remain independent. | Obtain NSE-3's physical final DSP-bootstrap publication or matching internal ROMs. Recover real COBBA mux/gain behavior and NHM-6/NHM-2 fitted analogue routes before physical-duplex promotion. Extend NHM-2 through encrypted degradation/save-state gates before claiming equal resilience coverage. Retain A5/2 as unsupported; extend handover validation to NHM-6/NHM-2 before claiming family-wide coverage. |
...
- The DSP/external-service peer contract is proved only for requests exercised by
  ...
  Dedicated-mode handover is independently gated on NSE-8 and NHM-5 for
  success, rollback and mid-procedure save/load; NHM-6/NHM-2 confirmation and
  broader inbound radio/L1 behavior remain open.
