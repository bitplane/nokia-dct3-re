...
| MBUS | `nokia_mbus_device` owns PUP offsets `0x18..0x1a`, receive/transmit holding state, physical single-wire TX echo, 9,600-baud character timing, FIQ2 byte events and the 423.1 Hz FIQ3 MBUSTIM source; ordinary v5.01/v6.00 boot initializes receive mode but transmits nothing. | Recover timer phase and differing-line collision/error behavior; attach a counterparty only for organic frames. |
