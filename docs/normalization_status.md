...
| DSP/network/external-service | `nokia_dspif_device` owns shared RAM/DSPIF, rings and interrupt-facing completion; `nokia_dsp_hle_device` owns the cross-ROM 64-exchange bootstrap publication and Nokia-specific boot/radio translation; `nokia_gsm_network_device` owns standards-shaped laboratory-cell identity and SI1--SI4; `nokia_external_service_peer_device` owns the separate request-driven service/test session. Firmware-programmed COBBA oscillator words `0x0ae/0x0b0/0x0b6` drive separate HLE audio voices; both 3210 ROMs organically prove the same 900 Hz start/stop sequence. | Recover physical bootstrap timing and the DSP-internal PLMN-measurement lifecycle; extend the GSM peer with MM/CC behavior only after its Nokia L1 transport entrance is evidenced; replace square-wave tone HLE with codec-backed waveform/amplitude behavior when evidenced. |
| MAD2 | `nokia_mad2_device` owns CTSI offsets `0x00..0x16`, timer-0/FIQ4, 15-bit timer-1/destination/FIQ5, reset request/cause, pending/masks, IRQ/FIQ aggregation and save-state restoration. SIMI clock bit 5 has a controller side effect; SIMI, MBUS and GENSIO are separate devices. | Establish extended IRQ ownership, exact physical timer dividers, remaining clock-gate bits and rail timing. |
