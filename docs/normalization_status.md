...
| --- | --- | --- |
| Hardware scenarios | `ADC_PROFILE`, `ADC0..7`, `CHARGER_ADC`, `SIM_ATR_HEX`, `SIM_CPHS_AOC` | Deterministic analog/card inputs, not inferred physical defaults. `CHARGER_ADC` is the raw selector-5 level driven while the physical charger input is asserted. |
