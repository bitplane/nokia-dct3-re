...
| MBUS | `nokia_mbus_device` owns PUP offsets `0x18..0x1a`, receive/transmit holding state, physical single-wire TX echo and wired-AND arbitration, 9,600-baud character timing, FIQ2 byte events and the one-shot 423.1 Hz FIQ3 MBUSTIM event. Ordinary v5.01/v6.00 boot initializes receive mode but transmits nothing; the NAM-2 profile attaches a product-local terminal only after its organic D0/01 request, and the complete D0/01-ACK-D0/04-D0/05-ACK exchange is gated. | Recover collision/error flags and electrical edge timing beyond the evidenced byte boundary. |
