...
| Diagnostic scenarios | `DIAG_RADIO_SCENARIO` | Active protocol stimuli which synthesize peer-owned packets through the real transport boundary. These are falsification/isolation instruments, never supported hardware defaults; each must have a named retirement condition. Scenarios 1/2 isolate the `NO_BCCH_LEFT`/`NO_PSW_FOUND` failure finalizers; scenarios 3--9 are bounded sequence falsifications awaiting deletion at the radio checkpoint; scenario 10 is the active ordered search/RSSI probe. |
