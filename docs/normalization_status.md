...
| CCONT/GENSIO | `nokia_ccont_device` owns selected-device registers and outputs; `nokia_gensio_device` owns serial selection/status, LCD pins and SELECT latches. The phone supplies raw ADC scenarios and the provisional watchdog clock. | Obtain board-level selector names, analog units, physical timing, RTC encoding, watchdog clock and SELECT peer identities from hardware evidence. |
