...
| CCONT/GENSIO | `nokia_ccont_device` owns selected-device registers and outputs; `nokia_gensio_device` owns serial selection/status, LCD pins and SELECT latches. The phone supplies raw ADC scenarios and the documented one-second watchdog tick. | Obtain board-level selector names, analog units, physical timing, RTC encoding and SELECT peer identities; recover the missing periodic firmware watchdog-service lifecycle. |
