...
| DSP/network/external-service | DSPIF, DSP HLE, Nokia L1 correlation, LAPDm, GSM session/network data and external service are separate owners. NSE-8, NHM-5 and NHM-6 independently pass paging, physical Answer/End, bidirectional GSM-FR, FACCH/BFI/SACCH coexistence and save-state replay; NSE-8 and NHM-5 additionally pass isolated host audio. Speech starts only when TCH, product-specific DSP control and an evidenced PCM profile agree; unknown products remain inert. | Obtain NSE-3's physical final DSP-bootstrap publication or matching internal ROMs. Recover real COBBA mux/gain behavior and NHM-6's fitted analogue routes before physical-duplex promotion. Add A5 only at the established burst-bit boundary, and extend sessions only from organic traffic. |
