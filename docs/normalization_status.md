...
- The 5210 v5.40 PPM E image is a validated interactive product profile. Its
  ...
  display-orientation, Family-A keypad and NSM-5 radio contracts. Ordinary and
...
| --- | --- | --- |
| SIM | `nokia_simi_device` owns MAD2 registers/FIFOs/IIR, the default character rate retained by firmware PPS `ff 00 ff`, FIQ6 UART delivery, status-bit-3 socket presence and FIQ7 card-detect edges. The callback-connected card owns T=0, declared file metadata and persistent mutable records. A shared typed subscriber profile supplies coherent identity, PLMN/operator and A3/A8 data to card and network. Both 3210 ROMs complete physical removal and fresh initialization after reinsertion. | Generalize timing beyond the current fixed ATR/PPS profile; recover ATR-start/turnaround and the physical sources of the typed timeout/parity causes. Extend commands only for organic requests or focused conformance tests; INVALIDATE/REHABILITATE remain absent because no retained trace requests them. |
...
| EEPROM | Product-owned serial EEPROMs on mapped PUP pins: 24C128 for NSE-8 and 24C64 for NSE-3. The 24C128 selects a conservative 5 ms busy cycle; mapped-pin page wrap, busy polling and two-process persistence pass. EEPROMSelX is unmapped after a five-ROM direct-access census disproved the old immutable-input alias. | Decode remaining fields, validate exact timing and make fallback-record extraction ROM-aware. |
...
| DSP/network/external-service | DSPIF, DSP HLE, Nokia L1 correlation, LAPDm, GSM session/network data and external service are separate owners. The generic network owns typed multi-cell topology, standards-shaped SI/control scheduling and RF conditions; the radio peer owns saved per-cell measurement/acquisition and dedicated-mode retune/access state while handset firmware owns selection, MM and handover completion. Same-/different-LAC reselection, replacement-cell paging, loss/recovery, preserved location and save-state gates pass across NSE-8/NHM-5/NHM-6/NHM-2. All four independently prove successful handover, timed rollback and mid-handover save/load under A5/0 and A5/1 with product-specific confirmation values; NHM-2 also retains physical call control and COBBA-GJP media across success. Generic A5/0/A5/1 operates only on the 114-bit post-interleave burst seam; product DSP-control, Nokia neighbour grammar and PCM evidence remain independent. | Obtain NSE-3's physical final DSP-bootstrap publication or matching internal ROMs. Recover real COBBA mux/gain behavior and NHM-6/NHM-2 fitted analogue routes before physical-duplex promotion. Extend NHM-2 through encrypted degraded-media parity before claiming equal resilience coverage. Retain A5/2 as unsupported. |
| MAD2 | `nokia_mad2_device` owns the CTSI registers within `0x00..0x16`, timer-0/FIQ4, timer-1 current/terminal-count/FIQ5, the 100 Hz FIQ8 centisecond source, reset request/cause, pending/masks, IRQ/FIQ aggregation, cross-ROM ninth-IRQ status/ack semantics, read-only external-status input `0x0e`, one-shot ARM clock-stop/routed wake and save-state restoration. PUP, KBGPIO, UIF, SIMI, MBUS and GENSIO are separate devices. | Establish the ninth IRQ's physical owner, `0x0e` pin ownership, exact physical timer dividers and transition latency, remaining clock-gate consumers and rail timing. |
| MBUS | `nokia_mbus_device` owns PUP offsets `0x18..0x1a`, receive/transmit holding state, physical single-wire TX echo, sampled peer-byte occupancy and wired-AND arbitration, 9,600-baud character timing, FIQ2 byte events and the one-shot 423.1 Hz FIQ3 MBUSTIM event. Ordinary v5.01/v6.00 boot initializes receive mode but transmits nothing; the NAM-2 profile attaches a product-local terminal only after its organic D0/01 request, and the pre-210 ms D0/01-ACK-D0/04-D0/05-ACK exchange is gated. | Recover differing-bit collision/error flags and electrical edge timing beyond the evidenced byte boundary. |
| Display/input | Native PCD8544, a cross-ROM LCD transport check, and extracted `nokia_kbgpio_device` matrix/IRQ0 behavior. Any unmasked column transition raises IRQ0; firmware masks row-drive transitions. The Lua mirror, handset key layout and timing remain acceptance/board fixtures. The generated EEPROM supplies fields authored by equivalent v6.00/v5.01 descriptor-`0x0749` reset constructors through normal NV loading. | Obtain an authentic configured `0x0749` profile; recover keypad debounce and display reset timing separately. |
...
| Vibrator/backlight | PUP bit 4 drives a named MAME `vibration` output; `0x1b.bits6..5` select four modes and bits 4..0 carry the mode parameter. A mapped-MMIO fixture validates the gate. Enabling `Vibrating alert` through the firmware UI and ringing an organic RTC alarm leaves both registers inactive, constraining that setting to another alert lifecycle rather than proving an output defect. The 3210 service manual establishes that COBBA drives separate LCD/key-light signals into the UI-Switch. Paired-ROM MAD2 traces and a complete changed-write census of MCU-visible DSP shared RAM find no key/timeout-specific output command. | Exercise vibra organically and map the mode/parameter fields to electrical behavior. Recover the lower DSP/COBBA light-control surface before exposing backlight outputs. |

The headless LCD mirror and delayed-key fixture are acceptance tooling in
`mame_nokia_dct3_input_exerciser.lua`.  Physical-End sequencing may wait for
either the session's diagnostic Alerting output or its already-connected-call
output, avoiding a missed sub-poll transition without changing CC state.
These outputs are observational and restored from saved generic session state;
the tooling does not add firmware shortcuts to the phone driver.
...
line state are registered. GSM cipher algorithm, Kc validity/value, activation
boundary and TDMA frame context are serialized; keystream registers are not
needed because generation is per burst. Immutable product settings remain
configuration. `make verify-mad2` retains the executable
