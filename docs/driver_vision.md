...
| DSP/DSPIF/audio | `nokia_dspif_device` owns shared RAM, packet rings and interrupt-facing completion. Paired v6.00/v5.01 code proves independent command-`0x08` speech (`0x0201`) and dedicated-channel (`0x0408`) fields; the DSP HLE applies the product-configured speech mask only while the radio owns TCH/F. GSM-FR frames cross a separate 20 ms codec boundary, then a distinct MAD2 device carries full-duplex PCM over the documented COBBA-clocked 520 kHz/8 kHz four-wire interface. The product-configured serial boundary models one sync clock, a 16-bit MSB-first sign-extended 13-bit converter word on falling edges, and 48 idle clocks. COBBA owns the 8 kHz receiver output and physical microphone input streams. A separate network-side voice peer proves non-silent decoded downlink across both ROMs, while an external host audio source proves non-silent microphone capture and network-decoded uplink without handset sample injection. | Recover COBBA's DSP-port register meanings and firmware-controlled analogue-mux contract. Implement A5 only at a real ciphered-bitstream boundary. |
