...
| MBUS | Extracted `nokia_mbus_device` with 9,600-baud RX/TX byte attachment, FIQ2 byte events and the 423.1 Hz FIQ3 MBUSTIM source; no default peer | Recover timer phase and single-wire arbitration, then attach a peer only when firmware organically transmits a supported frame. |
