...
screen. Today `make run` bakes in **32 `NOKI3210_*` knobs** (86 distinct knobs
exist in the source). That count is the **hack-debt gauge** — each knob substitutes
...
Current rough split of the 3,705-line driver:
...
- **~⅓ quarantined research scaffolding** (~1,200 lines): `flash_firmware_hooks`
  (~725), `ram_w/r_firmware_overrides` (~480), plus the forcing/experiment knobs.
  This is explicitly banner'd "NOT hardware behaviour" and is meant to **shrink to
  zero**.
...
**Today:** the **biggest hack cluster** — **12 `MBUS_D0_*` knobs** plus
`MBUS_RX_*`, `COMPLETE_MBUS_40_FRAME`, `MBUS_STATUS_W1C`. This is hand-tuned frame
*injection*, not a model: the boot's D0/D9 service frames are pushed in by timing
knobs rather than generated by a bus peer.
**Target:** a real **MBUS peer device** — a small state machine that speaks the D0/D9
service-frame protocol (now understood, see `service_bootstrap.md`) over the PUP MBUS
registers, raising the RX FIQ when a frame is ready. The firmware then drives the bus
instead of us racing it.
**Retires:** the entire `MBUS_D0_*` / `MBUS_RX_*` family — the single largest knob
reduction available.
...
the firmware path that makes #2/#4 observable, which unblocks #5. "Make it nice" is not a
separate refactor; it is the trail of deleted knobs left behind as each gate is cleared
faithfully. The gauge to watch is the `make run` knob count: **32 today**, and it should
fall step-wise as rows 1→5 land.
