...
|---|---|---|---|---|
| 1 | ~~**CCONT battery-measurement events**~~ → **`000d` = a missing service-transport peer reply (`task_285` = node-`0x18`); build attempted, practical bottom for this corpus** | `FORCE_000D_EVENTS`, `CCONT_EVENT15_DELAY` (diagnostics) | the **limp** (mode `000d`) | **peer identified; faithful fix NOT achievable on this corpus** (raw-disasm map + 6 failed levers). Gate needs standalone `0x15`; posted delayed-only, never reaches the mailbox — not starvation (fails at delay=1), not CCONT-IRQ (`r4=0`). Peer = the **service-transport / node-`0x18`** layer (`0xd5` = `service_transport_build_d5_notify`; retry spins at `0x21e00e` on `0xe2`/`0xe4`). Built a `MODEL_285_RESPONDER` — its spec address `0x285df8` never executes (inference on garbage decompile); suppressing the real `0x21e00e` spin left `0xd5`/`0x15` unchanged (transport spin isn't the `0xd5` producer; that producer is still unidentified). **Not hardware** (retracted). Closing it needs a cleaner reference (better decompile / working-phone trace), not another lever here. See `ccont_subsystem.md` (post-mortem). |
