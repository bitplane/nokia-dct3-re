...
| Startup/service/GSM peers | `nokia_external_service_peer_device` owns request-driven class-`0x40` service sessions; `nokia_radio_peer_device` owns Nokia L1 correlation, TDMA timing, per-cell measurement/acquisition state and the directional A5 burst seam behind one immutable protocol contract. Generic `gsm_xcch_l1` carries LAPDm through 184/456-bit coding and four SDCCH bursts. `nokia_lapdm_link_device` owns decoded bidirectional SAPI-0/SAPI-3 establishment, sequence and release state; `gsm_sms_transport` validates GSM 04.11 CP/RP messages against transaction data actually sent; `nokia_gsm_session_device` owns registration, paging, saved authentication/cipher activation, deterministic mobile-terminated/mobile-originated calls and bounded SMS Layer-3 state. `nokia_gsm_voice_peer_device` and the optional WebSocket adapter remain outside CC/RR and air L1. `nokia_gsm_network_device` owns standards-shaped multi-cell topology, SI1--SI4/control-channel data, RF availability, typed A5/0/A5/1, SI3 T3212 policy and typed SMS/Smart Message input compositions. Ordinary MT text now proves pre-page envelope validation, complete CP/RP/RR closure, firmware/SIM inbox ownership, physical read/delete, capacity, replay and two-message isolation on NSE-8, with independent NHM-5/NHM-2 application gates and an honest NHM-6 transport frontier. Organic same-/different-LAC reselection, loss/recovery, replacement-cell paging and independently closed two-part Smart Message transport pass across four products while firmware retains ranking, selection, MM, multipart retention, application dispatch and RTPL parsing ownership. NSE-8 proves RAM-owned reassembly and physical playback; NHM-5/NHM-2 independently corroborate playback and durable received-tone storage. A5/2 is rejected pending evidence. Shared behavior consumes typed contracts rather than handset branches. | Recover mobile-originated SMS, service-centre configuration and delivery reports; keep dedicated-mode handover separate until independently evidenced. |
