...
|---|---|---|---|---|
| 1 | ~~**CCONT battery-measurement events**~~ → **`000d` = a missing peer-response handshake (`task_285`), below corpus resolution** | `FORCE_000D_EVENTS`, `CCONT_EVENT15_DELAY` (diagnostics, not models) | the **limp** (mode `000d`) | **investigated to the boundary** (ROM disasm + runtime traces). Gate needs standalone `0x15`; it's posted delayed-only and never reaches the startup mailbox as a raw code — **not** starvation (proven: fails even at delay=1), not CCONT-IRQ (settles, `r4=0`). Upstream cause: the `0xd5` poll is a request/response machine (`task_285` @`0x11228c`) whose peer never answers on our peerless boot — same shape as CONTACT SERVICE, modellable *in principle*. But the peer/response/context is below the garbage-decompiled corpus's resolution; all forced stand-ins failed. **Not proven to need hardware** (retracted) — needs cleaner decompilation or a working-phone reference trace. See `ccont_subsystem.md`. |
