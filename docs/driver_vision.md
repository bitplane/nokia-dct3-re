...
| DSP/DSPIF/audio | `nokia_dspif_device` owns shared RAM, packet rings and interrupt-facing completion. Product firmware proves command-`0x08` speech (`0x0201`) independently from dedicated-channel control; one atomic DSP-HLE speech contract owns the parameter-command decoder and optional request predicate. NSE-3 can therefore retain its evidenced decoder without inventing a media-enable predicate. GSM-FR frames cross a separate 20 ms codec boundary, then a distinct MAD2 device carries full-duplex PCM over product-configured COBBA-clocked four-wire interfaces. NSE-8 uses the documented 520 kHz/8 kHz, 65-clock frame; NHM-5 independently uses 1 MHz/8 kHz and 125 clocks. Both documented profiles use one sync clock and a 16-bit MSB-first sign-extended 13-bit converter word sampled on falling edges. COBBA owns the 8 kHz receiver output and physical microphone streams. Network-side and isolated host-audio gates prove non-silent bidirectional GSM-FR, separate microphone capture and EAR playback for both product families. | Recover COBBA's DSP-port register meanings and firmware-controlled analogue-mux/gain contract. Implement A5 only at a real ciphered-bitstream boundary. |
