...
| Startup/service/GSM peers | `nokia_external_service_peer_device` owns request-driven class-`0x40` service sessions; `nokia_radio_peer_device` owns Nokia L1 correlation, TDMA timing, per-cell measurement/acquisition state and the directional A5 burst seam behind one immutable protocol contract. Generic `gsm_xcch_l1` carries LAPDm through 184/456-bit coding and four SDCCH bursts. `nokia_lapdm_link_device` owns decoded bidirectional SAPI-0/SAPI-3 establishment, sequence and release state; `nokia_gsm_session_device` owns registration, paging, saved authentication/cipher activation, deterministic mobile-terminated/mobile-originated calls and bounded SMS Layer-3 state. `nokia_gsm_voice_peer_device` and the optional WebSocket adapter remain outside CC/RR and air L1. `nokia_gsm_network_device` owns standards-shaped multi-cell topology, SI1--SI4/control-channel data, RF availability, typed A5/0/A5/1 and SI3 T3212 policy. Organic same-/different-LAC reselection, loss/recovery and paging on the replacement cell pass across four products while firmware retains ranking, selection and MM ownership. A5/2 is rejected pending evidence. Shared behavior consumes typed contracts rather than handset branches. | Recover the SMS CP/RP closing tail so the queued second Smart Message part can run; keep dedicated-mode handover separate until independently evidenced. |
