...
|---|---|---|---|---|
| 1 | ~~**CCONT battery-measurement events**~~ → **CONFIRMED WALL: the `000d` gate needs RTOS waiter/subscription state a provisioned boot sets up** | `FORCE_000D_EVENTS`, `CCONT_EVENT15_DELAY` (now labelled diagnostics, not models) | the **limp** (mode `000d`) | **done — proven a wall** (ROM disasm + `limp2_ecb` trace). The gate needs standalone `0x15`/`0x16`; the firmware posts them delayed-only, and their waiter-delivery branch is gated off in our faked boot (`mask&flags=0`, empty waiter list) → wheel-only `0xd5` loop, `0x15` never delivered even at delay=1. Not a buildable hardware model; same class as the channel-map wall, one gate earlier. See `ccont_subsystem.md` ("The `000d` wall"). |
