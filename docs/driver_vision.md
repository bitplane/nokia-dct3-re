...
| MBUS | Extracted `nokia_mbus_device` with 9,600-baud RX/TX, sampled peer-byte occupancy, FIQ2 byte events, one-shot 423.1 Hz FIQ3 terminal timing and wired-AND byte arbitration; NAM-2 alone attaches a request-derived M2BUS terminal | Recover differing-bit arbitration, electrical/error behavior and additional product-owned terminal protocols only from organic traffic. |
| DSP/DSPIF/audio | `nokia_dspif_device` owns shared RAM, packet rings and interrupt-facing completion. NSE-8, NHM-5, NHM-6 and NHM-2 independently prove command-`0x08` speech (`0x0201`) through organic calls; separately named product contracts retain those evidence identities even where values match. NSE-3 retains only its evidenced decoder without an invented media-enable predicate. GSM-FR frames cross a separate 20 ms codec boundary, then a distinct MAD2 device carries full-duplex PCM over product-configured COBBA-clocked four-wire interfaces. NSE-8 uses the documented 520 kHz/8 kHz, 65-clock frame; NHM-5, NHM-6 and NHM-2 independently select the documented COBBA-GJP 1 MHz/8 kHz, 125-clock shape. All use one sync clock and a 16-bit MSB-first sign-extended 13-bit converter word sampled on falling edges. COBBA owns analogue conversion. Internal-media gates prove non-silent bidirectional GSM-FR on all four products; isolated host microphone/EAR acceptance exists only where board routing is evidenced. | Recover COBBA's DSP-port register meanings and firmware-controlled analogue-mux/gain contract. Identify NHM-6/NHM-2 fitted analogue routes before physical-duplex promotion. Retain A5 at its implemented post-interleave burst boundary. |
| SIM | Separate `nokia_simi_device` controller using the default rate retained by firmware PPS `ff 00 ff`, and a callback-connected `nokia_sim_card_device` with declared files plus persistent ADN, SMS, SMS-parameter and CHV state. A shared typed subscriber profile owns coherent SIM/network identity and A3/A8 inputs. PIN-required startup, wrong-PIN exhaustion, PUK recovery, CHANGE, DISABLE and ENABLE execute organically through SIMI on the 3210; v5.01 independently corroborates startup VERIFY. SIMI status bit 3 plus MAD2 FIQ7 carry physical socket removal/reinsertion through firmware-owned teardown and fresh activation. | Generalize timing beyond the fixed ATR/PPS profile and recover the physical source/timing of the decoded timeout and parity/error causes. Broaden GSM 11.11 commands only from organic requests or focused conformance evidence. |
...
| Startup/service/GSM peers | `nokia_external_service_peer_device` owns request-driven class-`0x40` service sessions; `nokia_radio_peer_device` owns Nokia L1 correlation, TDMA timing, per-cell measurement/acquisition state, dedicated-mode retuning/access and the directional A5 burst seam behind one immutable protocol contract. Generic `gsm_xcch_l1` carries LAPDm through 184/456-bit coding and four SDCCH bursts. `nokia_lapdm_link_device` owns decoded bidirectional SAPI-0/SAPI-3 establishment, sequence and release state; `gsm_sms_transport` validates GSM 04.11 CP/RP messages against transaction data actually sent; `gsm_supplementary` owns bounded GSM 04.80 BER components; `nokia_gsm_session_device` owns registration, paging, saved authentication/cipher activation, handover outcome, deterministic mobile-terminated/mobile-originated calls, DTMF, two-leg call waiting/hold/swap/release, protocol-level multiparty state, call-independent supplementary-service sessions, and bounded SMS Layer-3 state. `nokia_gsm_voice_peer_device` and the optional WebSocket adapter remain outside CC/RR and air L1; a standalone endpoint proves organic call acceptance, bidirectional GSM-FR and local/remote release without owning the MAME process. `nokia_gsm_network_device` owns standards-shaped multi-cell topology, SI1--SI4/control-channel data, RF availability, typed A5/0/A5/1, SI3 T3212 policy and typed service inputs. NSE-8, NHM-5, NHM-6 and NHM-2 prove successful Handover Command/Physical Information/Handover Complete, timed rollback and mid-procedure save/load while firmware owns both channel configurations and the final outcome. Organic same-/different-LAC reselection, loss/recovery and replacement-cell paging pass across four products. A5/2 is rejected pending evidence. Shared behavior consumes typed contracts rather than handset branches. | Find a firmware product that organically originates multiparty FACILITY before promoting its protocol contract to runtime support, and keep any future SIP/RTP backend outside the handset and GSM state devices. |
