...
| MBUS | Extracted `nokia_mbus_device` with 9,600-baud RX/TX, FIQ2 byte events, one-shot 423.1 Hz FIQ3 terminal timing and wired-AND byte arbitration; NAM-2 alone attaches a request-derived M2BUS terminal | Recover bit-level arbitration, electrical/error behavior and additional product-owned terminal protocols only from organic traffic. |
