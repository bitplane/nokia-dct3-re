...
| SIM | Separate `nokia_simi_device` controller using the default rate retained by firmware PPS `ff 00 ff`, and a callback-connected `nokia_sim_card_device` with declared files plus persistent ADN, SMS, SMS-parameter and CHV state. PIN-required startup, wrong-PIN exhaustion, PUK recovery, CHANGE, DISABLE and ENABLE execute organically through SIMI on the 3210; v5.01 independently corroborates startup VERIFY. SIMI status bit 3 plus MAD2 FIQ7 carry physical socket removal/reinsertion through firmware-owned teardown and fresh activation. | Generalize timing beyond the fixed ATR/PPS profile; recover timeout and parity/error behavior; then extract reusable provisioning profiles and broaden GSM 11.11 conformance. |
...
| Startup/service/GSM peers | `nokia_external_service_peer_device` owns request-driven class-`0x40` service sessions; `nokia_radio_peer_device` owns Nokia L1 correlation, TDMA timing, per-cell measurement/acquisition state and the directional A5 burst seam behind one immutable protocol contract. Generic `gsm_xcch_l1` carries LAPDm through 184/456-bit coding and four SDCCH bursts. `nokia_lapdm_link_device` owns decoded bidirectional SAPI-0/SAPI-3 establishment, sequence and release state; `gsm_sms_transport` validates GSM 04.11 CP/RP messages against transaction data actually sent; `gsm_supplementary` owns bounded GSM 04.80 BER components; `nokia_gsm_session_device` owns registration, paging, saved authentication/cipher activation, deterministic mobile-terminated/mobile-originated calls, DTMF, two-leg call waiting/hold/swap/release, call-independent supplementary-service sessions, and bounded SMS Layer-3 state. `nokia_gsm_voice_peer_device` and the optional WebSocket adapter remain outside CC/RR and air L1; a standalone endpoint proves organic call acceptance, bidirectional GSM-FR and local/remote release without owning the MAME process. `nokia_gsm_network_device` owns standards-shaped multi-cell topology, SI1--SI4/control-channel data, RF availability, typed A5/0/A5/1, SI3 T3212 policy and typed SMS/Smart Message input compositions. Ordinary MT text now proves pre-page envelope validation, complete CP/RP/RR closure, firmware/SIM inbox ownership, physical read/delete, capacity, replay and two-message isolation on NSE-8, with independent NHM-5/NHM-2 application gates and an honest NHM-6 transport frontier. Organic MO text covers physical composition, service-centre configuration, CP timeout/retry, reject outcomes, requested delivery reports and SIM persistence across the four validated products. Organic same-/different-LAC reselection, loss/recovery, replacement-cell paging and independently closed two-part Smart Message transport pass across four products while firmware retains ranking, selection, MM, multipart retention, application dispatch and RTPL parsing ownership. NSE-8 proves RAM-owned reassembly and physical playback; NHM-5/NHM-2 independently corroborate playback and durable received-tone storage. NSE-8 EMS proves transport, read/delete and UCS-2 presentation while ignoring Text Formatting IE semantics. A5/2 is rejected pending evidence. Shared behavior consumes typed contracts rather than handset branches. | Extend independently evidenced services such as Cell Broadcast, multiparty control and dedicated-mode handover. Keep any future SIP/RTP backend outside the handset and GSM state devices. |
