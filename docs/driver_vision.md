...
| DSP/DSPIF | `nokia_dspif_device` owns shared RAM, packet rings and interrupt-facing completion; `nokia_dsp_hle_device` owns cross-ROM bootstrap and service timing. The SC=0 service entrance organically exposes one-way cipher-control type `0x14`; an answered-call census exposes empty TCH polls but no speech packet family. The same run proves answer-only committed shared-control command `0x08/0x060b` before Connect Ack and a separate bounded 900 Hz acknowledgement tone through the existing two HLE voices. No cipher, command-`0x08`, PCM or codec backend exists. | Decode answer-only command `0x08/0x060b` and the DSP-to-COBBA PCM consequence from firmware or a physical protocol source; implement A5 only at a real ciphered-bitstream boundary. |
