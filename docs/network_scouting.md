...
the firmware security and unbacked clock editors with physical keys, waits on
the read-only CC Alerting publication, and uses ordinary Navi matrix presses
for Answer and End. The firmware emits Connect, Disconnect and Release
Complete, the radio uses NHM-6's `0x14` release transaction, and the DSP
independently publishes `0x860b` then `0x840a`.

The lifecycle now also requires sustained bidirectional GSM-FR across NHM-6's
own COBBA-GJP PCM declaration. `make verify-3330-radio-media-resilience`
repeats the preserved-PMM call with independent uplink/downlink burst
