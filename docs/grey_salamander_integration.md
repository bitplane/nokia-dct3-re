...
| GS-08 | Mobile-terminated call ordering from paging through Setup, Alerting, Connect and clearing | incoming-service queue and GSM call-control session | The named call fixture now proves page, MM Information, SETUP, organic Call Confirmed/Alerting/Disconnect, network Release, organic Release Complete and RR teardown. The default erased-identity UI clears the call and no TCH/speech is claimed; Connect handling is retained for a later answered-call fixture | **landed, bounded signalling** |
