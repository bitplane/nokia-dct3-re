...
| GS-14 | Calls and SMS should produce asynchronous request objects and accept later host decisions | future backend-neutral event interface | MO calls now expose a saved monotonic request ID and decoded called digits, and the no-backend policy reproducibly selects connect, busy, no-answer or service reject. Queue later host decisions onto the emulation scheduler without moving CC/RR sequencing out of the session; MO SMS requests remain future work. | **request identity and deterministic policy landed; asynchronous routing queued** |
