...
| GS-15 | Paging and dedicated-channel work require deadlines and ordered queues | radio scheduler and LAPDm timers | Paging groups use saved frame time. LAPDm establishment and acknowledged downlinks now use a saved one-second SDCCH T200, exact-frame replay and N200=3; terminal failure retires the dedicated transaction while preserving registered idle MM identity. Queued host-service admission deadlines remain part of the host-boundary work rather than link state | **LAPDm expiry landed; host queue deadlines remain** |
