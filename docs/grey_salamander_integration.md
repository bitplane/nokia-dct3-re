...
| GS-09 | Mobile-originated SMS uses CP-DATA/ACK and RP-DATA/ACK/error, with GSM-7 and UCS-2 decoding | GSM SMS session and host request event | NSE-8 physical composition now reaches MM service type 4, mobile SAPI-3 establishment, uplink reassembly, parsed SMS-SUBMIT, CP/RP acknowledgement, `Message sent`, and final SAPI-0 RR release. Failure/retry policy, host requests and cross-product evidence remain open. | **successful local submission landed** |
