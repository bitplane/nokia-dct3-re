...
| GS-12 | Port-addressed 8-bit SMS and concatenation carry Nokia Smart Messaging payloads | SMS TPDU/user-data codec above GS-10 | The bounded 251-byte RTPL tone uses 128-byte multipart capacity, shared reference `7a`, two queued parts and independent RP references. Both parts now pass exact stop-and-wait SAPI-3 delivery, handset CP/RP response, network CP-ACK and RR release through separate pages on NSE-8, NHM-5, NHM-6 and NHM-2. `EF_SMS` stays free. Save/load replay covers both closing exchanges and the queued boundary; visible reassembly, save/play and persistence are not claimed | **sequential two-part transport landed; firmware application behavior pending** |
