...
| MBUS `18..1a` | extracted byte controller, status, RX/TX attachment, physical single-wire TX echo, sampled peer-byte occupancy, FIQ2 byte events and 423.1 Hz FIQ3 MBUSTIM source | Cross-ROM partial, physical character and timer rates | Both 3210 ROMs share initialization and idle behavior; focused RX proves status-bit-5/control-bit-6/FIQ2 delivery, while NAM-2 exercises FIQ3 transaction start and the complete request/terminal exchange in its recovered pre-210 ms phase. Recover differing-bit collision and error behavior. |
