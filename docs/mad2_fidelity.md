...
| MBUS `18..1a` | extracted byte controller, status, RX/TX attachment, FIQ2 byte events and 423.1 Hz FIQ3 MBUSTIM source | Cross-ROM partial, physical character and timer rates | Both 3210 ROMs share initialization and idle behavior; focused RX proves status-bit-5/control-bit-6/FIQ2 delivery, while NAM-2 exercises the FIQ3 transaction start. Recover timer phase, collision and error behavior. |
