...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Nokia 3210 NSE-8 v6.00 | Yes | Yes | Yes | Yes | Yes | Yes | Partial | Structural, navigation, authentication, mobile-terminated and physical-dial mobile-originated call lifecycles, paired GSM-FR/FACCH/degraded media and isolated physical audio pass. The outgoing host suite additionally proves asynchronous outcomes, alerting/traffic termination, physical-End isolation, non-silent physical GSM-FR loopback, overload/reconnect, sequential calls and save/load before decision, during alerting, traffic/media and CC/RR release. Physical long-power shutdown reaches CCONT rail-off after save/load. Real COBBA DSP-controlled mux/gain semantics remain opaque. |
...
| Nokia 3310 NHM-5 v6.39 | Yes | Yes | Yes | Yes | Yes | Yes | Partial | Frontier/navigation, authentication, registration, paging, incoming and physical-dial outgoing calls, media resilience and physical duplex pass using its product-specific radio, DSP-control and 1 MHz/125-clock PCM contracts. Focused host promotion proves correlated acceptance and remote clearing through the shared adapter/session. Product-specific analogue gain programming remains unproved. |
| Nokia 3330 NHM-6 v4.50E | Yes | Yes | Yes | Yes | Yes | No | Partial | Fresh-PMM frontier, navigation, registration and paging gates pass. Preserved-PMM incoming and physical-dial outgoing calls complete through physical Navi Answer/End and carry bidirectional GSM-FR over NHM-6's independently evidenced command-`0x08` speech field and COBBA-GJP 1 MHz/125-clock PCM link. Preserved-PMM host promotion proves correlated acceptance and remote clearing through the shared adapter/session. FACCH substitution, degraded frames, concealment, SACCH and active-call replay pass before PCH. The fitted microphone input remains unknown, so physical duplex is unpromoted. |
| Nokia 3410 NHM-2 v5.46E | Yes | Yes | Yes | Yes | Yes | No | Partial | Fresh/preserved-PMM registration, paging and assigned-SDCCH replay pass. Incoming calls prove ringing, physical Send/End and assignment idempotence; physical-dial outgoing calls prove called-number SETUP, one assignment, Connect/Acknowledge, sustained media and NHM-2's release tail. Principal host promotion carries 200 correlated GSM-FR frames through TCH/F and remote clearing. NHM-2 independently publishes speech `0x060b`/`0x040a`; its typed 1 MHz/8 kHz 13-in-16 PCM profile sustains bidirectional GSM-FR. Fitted analogue routes remain unestablished, so physical duplex is not promoted. |
