...
| Nokia 2100 NAM-2 v5.84 PPM E | Partial | No | No | No | No | No | No | `verify-2100-frontier` proves reset, 16-Mbit flash execution, RAM/MAD2 access, product-local GENSIO/CCONT and display contracts, a 64-exchange DSP bootstrap, service discovery and application registration. The compact type-74 completion clears `CONTACT SERVICE` to an exact blank 96x65 oracle. NAM-2's physical MBUS request, 423.1 Hz FIQ3 source and documented terminal `D0/04` -> phone `D0/05` application exchange are established; timer phase and single-wire arbitration remain unavailable, so the exchange cannot yet be retained as a reproducible model. Its five-row keypad scan is organic but remains masked at this boundary, and SIM remains unpromoted. |
| Nokia 3610 NAM-1 v5.11 PPM E | No | No | No | No | No | No | No | All supplied MCU/PPM streams validate; PPM E is a hash-pinned representative, not a hardware preference. Static evidence recovers 670 direct MAD2 accesses, PUP/keypad/UIF regions and the exact 18-entry CCONT vocabulary, but no direct GENSIO SELECT access. Product wiring, display, DSP contract and persistent storage remain unresolved, so no executable profile is declared. |
| Nokia 5210 NSM-5 v5.40 PPM E | Partial | No | No | No | No | No | No | The hash-pinned local Wintesla set reaches a stable firmware-rendered CONTACT SERVICE frame. Its organic CCONT transaction proves an NSM-5 GENSIO distinction: control `0x22` leaves bit 2 clear, so receive-ready must follow the byte write rather than the NSE-8 selection bit. With that correction the DSP upload and LCD transport run; DSP/service bootstrap semantics, display scan orientation, SIM bring-up and the failing self-test remain unpromoted. |
