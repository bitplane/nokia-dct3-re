...
| Nokia 6110 NSE-3 family | No | No | No | No | No | No | No | The declared `noki6110` machine now expresses the manual-backed 1 MiB flash extent as the independently reported Intel 28F800B3-T (`0089:8892`), plus a 64 KiB SRAM map, serial 8 KiB 24C64, 84 x 48 display, dedicated UE4 five-by-five keypad, MIC2/EAR routes and 1 MHz/8 kHz 13-in-16 PCM profile. Its ROM3 boot/DSP, flash and EEPROM inputs remain `NO_DUMP`; direct-to-flash reset bypass and all firmware-derived peers remain disabled. No local hashed ROM3/ROM4 v5.48 baseline exists, so this is a static hardware declaration—not a runtime promotion. See `docs/6110_bringup.md`. |
