...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Nokia 3210 NSE-8 v6.00 | Yes | Yes | Yes | Yes | Yes | Yes | Partial | Structural, navigation, authentication, mobile-terminated and physical-dial mobile-originated call lifecycles, active-call save-state replay, paired GSM-FR/FACCH/degraded media and isolated physical audio pass. Real COBBA DSP-controlled mux/gain semantics remain opaque. |
...
| Nokia 3310 NHM-5 v6.39 | Yes | Yes | Yes | Yes | Yes | Yes | Partial | Frontier/navigation, authentication, registration, paging, incoming and physical-dial outgoing calls, media resilience and physical duplex pass using its product-specific radio, DSP-control and 1 MHz/125-clock PCM contracts. Product-specific analogue gain programming remains unproved. |
| Nokia 3330 NHM-6 v4.50E | Yes | Yes | Yes | Yes | Yes | No | Partial | Fresh-PMM frontier, navigation, registration and paging gates pass. Preserved-PMM incoming and physical-dial outgoing calls complete through physical Navi Answer/End and carry bidirectional GSM-FR over NHM-6's independently evidenced command-`0x08` speech field and COBBA-GJP 1 MHz/125-clock PCM link. FACCH substitution, bidirectional degraded frames, concealment, SACCH coexistence and exact active-call save-state replay pass before clean return to PCH. The fitted microphone input remains unknown, so physical duplex is unpromoted. |
| Nokia 3410 NHM-2 v5.46E | Yes | Yes | Yes | Yes | Yes | No | Partial | Fresh and preserved-PMM gates prove registration, paging and save/load across acquisition and assigned SDCCH. Incoming calls prove firmware-driven ringing, physical Send/End and repeated-Call-Confirmed assignment idempotence; physical-dial outgoing calls prove CM Service, called-number SETUP, one assignment, Connect/Acknowledge, sustained media and NHM-2's distinct release tail. Both directions return to PCH. NHM-2 independently publishes command-`0x08` speech values `0x060b`/`0x040a`; its typed COBBA-GJP profile sustains timed 1 MHz/8 kHz 13-in-16 PCM and bidirectional GSM-FR. Negative cell, assignment and paging gates remain. Fitted analogue routes remain unestablished, so physical duplex is not promoted. |
