...
| Nokia 6110 NSE-3 v5.48 PPM B (ROM3) | No | No | No | No | No | No | No | The original service package, manifest ROM selection, normalized 1 MiB image and independent real-ROM3 handset log agree on `V  5.48`, `08-09-99`, DSP software `40.3.617`, ROM3 and physical COBBA B07. `verify-6110-v548-static` proves a four-cell bootstrap with a distinct pre-upload `0x10004/0x10006` exchange, `0xffff` sentinels, 64 transfer blocks and final `0x10000/0x10002` capture. It also proves the first captured result is rendered as `B06` and later required to equal `0x0b06`; the conflict with physical B07 prevents assigning that result a fitted-silicon meaning. The pre-upload values, second final value, F711604 internal ROMs and EEPROM are absent, so the BIOS selects an unresolved fail-closed profile and is not promoted to booting. |
| Nokia 6110 NSE-3 v05.48 PPM B (ROM4) | No | No | No | No | No | No | No | The package manifest explicitly names this separate `Rom4ImageFile`/`Rom4PpmFile` family; its normalized image embeds `V 05.48`, `03-09-99`. The static gate proves the homologous loader at a `0x1c1c` relocation and a different staged-stream fingerprint, plus the same final `0x0b06` constraint at separately relocated formatter and comparison sites. Its pre-upload values and second final result remain unresolved. ROM4 internal MAD2/DSP images are declared separately `NO_DUMP`; no ROM3 internal image or pre-upload publication is inherited. |
