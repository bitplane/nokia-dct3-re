...
| Nokia 6110 NSE-3 v4.06 PPM B (ROM3 candidate) | No | No | No | No | No | No | No | The declared `noki6110` machine expresses the manual-backed 1 MiB flash extent as the independently reported Intel 28F800B3-T (`0089:8892`), plus a 64 KiB SRAM map, serial 8 KiB 24C64, 84 x 48 display, dedicated UE4 five-by-five keypad, MIC2/EAR routes and 1 MHz/8 kHz 13-in-16 PCM profile. An Internet Archive service package now normalizes reproducibly to an exact 1 MiB v4.06 MCU + PPM B image (`5025a6ac…`), but its ROM3 compatibility remains a labelled candidate and the matching F711604 boot/DSP ROMs and EEPROM remain `NO_DUMP`. Direct-to-flash reset bypass and all firmware-derived peers remain disabled, so this is not a boot promotion. See `docs/6110_bringup.md`. |
