...
| Nokia 2100 NAM-2 v5.84 PPM E | Partial | No | No | No | No | No | No | `verify-2100-frontier` proves reset, 16-Mbit flash execution, RAM/MAD2 access, an independently observed `0x2c/0x2d/0x6c/0x6d` GENSIO/CCONT route, and sustained firmware LCD traffic reaching an exact 96x65 `CONTACT SERVICE` oracle without DSP FIQ0. NAM-2's bank range, 96-byte transfers, documented panel geometry and recovered scan direction establish the display contract. A complete v5.21 image with populated PMM converges on the same frame, so the next boundary is product-local DSP/service completion; keypad and SIM remain unpromoted. |
