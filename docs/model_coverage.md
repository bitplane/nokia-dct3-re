...
| Nokia 6110 NSE-3 v4.06 PPM B (ROM3 candidate) | No | No | No | No | No | No | No | The declared `noki6110` machine expresses the manual-backed 1 MiB flash extent as the independently reported Intel 28F800B3-T (`0089:8892`), plus a 64 KiB SRAM map, serial 8 KiB 24C64, 84 x 48 display, firmware-verified UE4 five-by-five keypad, MIC2/EAR routes and 1 MHz/8 kHz 13-in-16 PCM profile. Exact-image static gates now cover reset/vector/SRAM, direct MAD2 extent, keypad, GenIO bit-2 EEPROM clock/two-byte address framing, the generic MAD2 SIMI controller surface, generic DSPIF ring geometry, and the MCU's 64-block bootstrap transfer. The latter samples 32,766 strided flash halfwords, appends two terminators and stages a fingerprinted 64 KiB stream while alternating two synchronization cells. Firmware captures final shared words `0x10000/0x10002`; it renders the first as `B06` and one later path requires `0x0b06`, proving the generic HLE ready word `1` is incompatible. This still does not establish that the stream is DSP code, its DSP destination, the second result, broader response meanings or a service grammar. The synthetic SIM card remains separately disabled; the v4.06 image (`5025a6ac…`) remains a labelled ROM3 candidate; matching F711604 boot/DSP ROMs and EEPROM remain `NO_DUMP`; and every firmware-derived peer is disabled. Direct-to-flash reset bypass remains disabled, so this is not a boot promotion. See `docs/6110_bringup.md`. |
