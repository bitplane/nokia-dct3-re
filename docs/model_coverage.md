...
| Nokia 2100 NAM-2 v5.84 PPM E | Partial | No | No | No | No | No | No | `verify-2100-frontier` proves reset, 16-Mbit flash execution, RAM/MAD2 access, product-local GENSIO/CCONT and display contracts, a 64-exchange DSP bootstrap, service discovery and application registration. The compact type-74 completion clears `CONTACT SERVICE` to an exact blank 96x65 oracle. NAM-2's physical MBUS request and documented terminal `D0/04` -> phone `D0/05` application exchange are established; MAD2 FIQ3 timing and single-wire arbitration remain unavailable, so the exchange cannot yet be retained as a reproducible model. Its five-row keypad scan is organic but remains masked at this boundary, and SIM remains unpromoted. |
