...
| Nokia 3330 NHM-6 v4.50E | Yes | Yes | Yes | Yes | No | No | Partial | Fresh-PMM frontier, navigation, registration and paging gates pass. `verify-3330-radio-incoming-call-lifecycle` cold-boots preserved PMM, clears the firmware security and clock editors through physical keys, completes paging/ciphering/TCH assignment, presents the incoming call, answers and ends it through the physical Navi key, and returns organically to PCH. NHM-6 independently emits the `0x860b`/`0x840a` command-`0x08` speech-control wire. No NHM-6 PCM timing, analogue topology, ringtone output or bidirectional media is inferred from NHM-5, so those levels remain unpromoted. |
