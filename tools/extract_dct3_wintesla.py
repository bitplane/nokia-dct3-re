...
def main() -> None:
    ...
    if args.expect_eeprom_sha1 and not args.pmm:
        parser.error("--expect-eeprom-sha1 requires --pmm")
    ...
    if args.pmm:
        ...
        if args.expect_eeprom_sha1 and eeprom_sha1 != args.expect_eeprom_sha1.lower():
            raise ValueError(
                f"EEPROM SHA-1 mismatch: expected {args.expect_eeprom_sha1}, "
                f"got {eeprom_sha1}"
            )
    ...
    summary = f"wrote {args.flash_output} ({len(flash):#x}, sha1 {flash_sha1})"
    if pmm is not None:
        args.eeprom_output.parent.mkdir(parents=True, exist_ok=True)
        args.eeprom_output.write_bytes(pmm)
        summary += (
            f"\nwrote {args.eeprom_output} ({len(pmm):#x}, "
            f"sha1 {eeprom_sha1})"
        )
