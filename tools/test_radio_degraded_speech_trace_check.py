...
class RadioDegradedSpeechTraceCheckTests(unittest.TestCase):
    ...
    def test_accepts_bad_frame_and_recovery(self):
        self.assertIn("observed 1", self.run_check(self.good_lines()))

    def test_discards_save_state_reference_branch(self):
        lines = self.good_lines()
        lines[5:5] = [
            "state_replay: phase=reference event=begin t=10.050000",
            "radio_l1: direction=uplink impairment=invert-data burst=1 count=99 fn=90 t=10.060000",
            "state_replay: phase=reference event=end t=10.070000",
            "state_replay: phase=restored event=begin t=10.050000",
        ]
        self.assertIn("observed 1", self.run_check(lines))

    def test_accepts_recovery_before_final_teardown_edge_loss(self):
        lines = self.good_lines()
        lines.extend([
            "radio_l1: direction=downlink kind=speech good=0 count=2 fn=200 t=12.030000",
        ])
